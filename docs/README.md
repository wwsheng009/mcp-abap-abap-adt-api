...
│   │   ├── AuthHandlers.ts                   # 身份验证 (3个工具)
│   │   ├── ObjectHandlers.ts                 # 对象操作 (5个工具)
│   │   ├── TransportHandlers.ts              # 传输管理 (15个工具)
│   │   ├── ObjectLockHandlers.ts             # 对象锁定 (2个工具)
│   │   ├── ObjectSourceHandlers.ts           # 源代码操作 (2个工具)
│   │   ├── ObjectManagementHandlers.ts       # 对象管理 (3个工具)
│   │   ├── ObjectDeletionHandlers.ts         # 对象删除 (1个工具)
│   │   ├── ObjectRegistrationHandlers.ts     # 对象注册 (3个工具)
│   │   ├── ClassHandlers.ts                  # 类操作 (2个工具)
│   │   ├── CodeAnalysisHandlers.ts           # 代码分析 (15个工具)
│   │   ├── PrettyPrinterHandlers.ts          # 代码格式化 (3个工具)
│   │   ├── UnitTestHandlers.ts               # 单元测试 (4个工具)
│   │   ├── GitHandlers.ts                    # Git集成 (10个工具)
│   │   ├── DdicHandlers.ts                   # 数据字典 (4个工具)
│   │   ├── ServiceBindingHandlers.ts         # 服务绑定 (3个工具)
│   │   ├── QueryHandlers.ts                  # 数据库查询 (2个工具)
│   │   ├── FeedHandlers.ts                   # Feed和Dump (2个工具)
│   │   ├── DebugHandlers.ts                  # 调试器 (13个工具)
│   │   ├── NodeHandlers.ts                   # 节点操作 (2个工具)
│   │   ├── DiscoveryHandlers.ts              # ADT发现 (7个工具)
│   │   ├── RenameHandlers.ts                 # 重命名重构 (3个工具)
│   │   ├── AtcHandlers.ts                    # ATC检查 (10个工具)
│   │   ├── TraceHandlers.ts                  # 性能追踪 (10个工具)
│   │   ├── RefactorHandlers.ts               # 重构操作 (3个工具)
│   │   └── RevisionHandlers.ts               # 版本历史 (1个工具)
│   ├── lib/                                  # 工具库
│   │   └── logger.ts                         # 日志系统
│   └── types/                                # 类型定义
│       └── tools.ts                          # 工具类型接口
